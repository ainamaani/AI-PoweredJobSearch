...
const handleJobsRecommendations = async (req: Request, res: Response) => {
    try {
        ...
        const relevantJobPostings = jobPostings.filter(job => {
            // Check if the job title contains any keyword from the user input
            const jobTitleKeywords = job.title.toLowerCase().split(' ');
            const userInputKeywords = userInput.toLowerCase().split(' ');
            return userInputKeywords.some((keyword : any) => jobTitleKeywords.includes(keyword));
        });
        ...
        if (!stemmedJobCorpus || stemmedJobCorpus.length === 0) {
            ...
            return res.status(400).json({ error: 'Stemmed job corpus is null or empty' });
        }
